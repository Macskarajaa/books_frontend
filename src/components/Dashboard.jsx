...
export const Dashboard = () => {
  ...
  const rows = books.map((obj) => (
    <Table.Tr key={obj.id}>
      ...
      <Table.Td>
        ...
        <TiPencil size="20px" color='blue' style={{ cursor: "pointer" }} onClick={() => handleEdit(obj)} />
      </Table.Td>
    </Table.Tr>
  ...
  const handleSave = async () => {
    try {
      if (editingBook) {
        console.log("change");
        const updateBook = await updateBook(editingBook.id, newBook)
        setBooks(prev => prev.map(obj => obj.id == editingBook.id ? updateBook : obj))
  ...
  return (
    <>
      <Flex direction='column' gap='md' justify='flex-start' align='center'>
        ...
        <Button onClick={() => setShowForm(true)}>Hozzáadás</Button>
      </Flex>
